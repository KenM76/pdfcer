...
4v. **★★ PER-EDIT ERRATUM ATTRIBUTION LIVES IN THE **RAW HTML** OF `pdf-issues.pdfa.org` — `data-issue` / `data-iso`. FLATTENING DESTROYS IT.** Established 2026-09-15 (ISO 32000-2 §12.5.2 ignore-list). The standard flatten-and-grep recipe (preserve `[INS]`/`[DEL]`, strip tags) shows *what* changed and hides *who* changed it. Every `<ins>`/`<del>` actually carries `onmouseenter="mouseEnter(this)" data-issue="56" data-iso="approved"`, and `data-issue` may be a **comma list** (`"23,34"`). **One paragraph routinely carries edits from three different errata** — the §12.5.2 paragraph's `MK` insertion is #56 while its `BM` deletion and its lead-in rewrite are #23+#34 and its new NOTE is #34. Recipe: locate the anchor string with `s.find(...)` on the **undecorated** HTML and print ± 2-3 kB around it. Corroborate in the staged EC3 PDF: the markup annot's `/T` is literally `Issue #56` or `Issue #23 and #34`, and **`/QuadPoints` recovery (item 3-ISO2) proves which glyph run a `/StrikeOut` covers** — it returned exactly `'BM, '` here, converting an inference into a measurement. **Never attribute an erratum by the paragraph it lands in.**

4w. **THE `pdf-issues` `README.md` IS THE AUTHORITY ON ITS OWN LABELS, and it is free + `curl`-able** — `raw.githubusercontent.com/pdf-association/pdf-issues/main/README.md` (HTTP 200, ~11 kB). `ISO approved` = approved by **both** the PDF Association TWG **and** the responsible ISO working group; `ISO submitted` = TWG-approved, awaiting ISO; **no logo** = TWG only. The rendered errata index adds that closed errata are applied to *"the master documents that form the basis for future editions of ISO 32000"* ⇒ **approved ≠ published**. Cheap corroborating ratio, via `search/issues?q=repo:…+label:"<L>"&per_page=1` → `total_count`: **311** `ISO approved` vs **9** `ISO 32000-2 Amd1` (2026-09-15), so absence from `Amd1` is not a demotion.
