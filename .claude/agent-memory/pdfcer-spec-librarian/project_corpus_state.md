...
## 82. "I will IMPLEMENT a writer directly from your reply" — an ARR primary (2026-09-23, G033 bare CFF)

Shape: "fill GAP X; the source is all-rights-reserved; give every byte value in the reply".

### 82a. Build the worked example with a SCRIPT and round-trip it through an independent parser
The fact sheet's §10 is a 120-byte CFF generated by `mkcff.py`-style code and decoded by fontTools (installed: 4.61.1), not hand-assembled. This catches wrong opcode bytes, offset arithmetic and operand-form mistakes that citing alone cannot catch (the ★★ hand-computed-derived-value rule). Label it `measured` and say which validator was NOT run (no OTS binary here).

### 82b. An all-rights-reserved free download = paraphrase-only, zero verbatim, own LEGAL_NOTE row
Adobe TN5176/5177: staged under `_sources\adobe_tn\`, registered in LEGAL_NOTE + index (two registrations). Byte values and operator numbers are facts; sentences are not. Put quotable corroboration (the MS OpenType CFF chapter) in the flat-grep index instead.

### 82c. The spec's layout is silent on WRITER strategy — source a writer
TN5176 never says how to break the Top-DICT-offset circularity. Two routes: the 5-byte `1d` operand form (legal because no shortest-form rule exists), or fontTools `CFFWriter.toFile`'s fixed-point loop. Cite the writer's code for the second route; label the first DERIVED.
