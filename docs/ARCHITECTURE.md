...
**What was wrong, and where it had already been written down three times.** `plan_move_text_run`'s own doc comment, `text_edit/format.rs`, and `text_edit/reflow_apply.rs` each independently recorded "splitting the text object would discard `Tm` (§9.4.1)" as a reason to prefer rewriting an existing operator over restructuring the stream — all three reached that sentence while looking for a way to wrap `q`/`Q` around a text edit, a different problem with a genuinely relevant cost. The sentence is true and was never an obstacle: restating `Tm` costs one operator, and the run being split already carries its own matrix (it must, to be addressable at all). Nobody had priced it, because nobody needed to for the problem those three sites were actually solving.

**What makes the true cost this low.** ISO 32000-1 §9.3/§9.4.1: `BT` and `ET` bracket a text object and reset ONLY the text matrix and text line matrix (`Tm`/`Tlm`) to identity at `BT`. Every other piece of graphics/text state — the font/size (`Tf`), character/word spacing (`Tc`/`Tw`), horizontal scaling (`Tz`), leading (`TL`), rise (`Ts`), rendering mode (`Tr`), fill/stroke colour, and the CTM — is **not** reset by `ET`/`BT` and needs no restatement across the split. This is the exact contrast with `text_edit/reflow_apply.rs`'s own restore-by-value machinery (R88, decision from the FF-H slice, `ARCHITECTURE.md` body above): that code rebuilds a text object's *contents* and therefore owes a full `pre | set_ops | mid | restore_ops | post` splice; `split_text_object` rewrites nothing inside either resulting object and owes none of it.

**The correctness precondition, stated so a future extension of this technique inherits it rather than re-deriving it.** A run is only safely split-before if it was placed by an operator that sets `Tm` and `Tlm` to the SAME value (Table 108 — `Tm`, `Td`, `TD`, and the implicit `BT`-origin case all qualify; a run whose position is *inherited* from an earlier operator's advance, with no `Tm`/`Tlm`-setting operator of its own, does not, and is refused by name: `SplitRunInheritsPosition`). One restated matrix then serves both roles the new `BT` needs it for.

**Body-section effects.** No existing body section states or depends on the now-corrected "splitting is expensive" premise — it lived only in code doc comments (`pdfcer-core`, out of this document's remit), not in `ARCHITECTURE.md` itself, so there is nothing here to amend. This entry exists so a future decision reasoning about text-object restructuring finds the correct price already recorded, rather than re-deriving it or re-citing the same three now-corrected doc comments.

**Not minted as a standing rule.** Three same-decision instances of "a cost was recorded and never priced" would ordinarily clear this project's own two-occurrence bar (see decision 156's `R256`), but all three sites were reached and corrected within this one Pass, by the same author, for the same reason — one occurrence with three symptoms, not three independent ones. Watched for a genuinely separate future instance rather than named now.

**Sourcing (hard rule 8) — no shell this filing.** Taken from the requesting engineer's own report. Independently confirmed against live source via `Read`/`Grep`: `crates/pdfcer-core/src/vector/edit.rs` (`plan_split_text_object`'s doc comment, `SplitRunInheritsPosition`), `crates/pdfcer-core/src/text_edit/format.rs`, `crates/pdfcer-core/src/text_edit/reflow_apply.rs` (the three sites' own "would discard `Tm`" reasoning). Commit hash and full gate results not independently verified — no shell.

**Decision ceiling: `156` → `157`**, next free `158`. **Standing rules ceiling: `R257` used (corrected this filing from a stale carried "next free `R257`" — see `ROADMAP.md`'s 554th-filing Ledger note), next free `R258`.** No standing rule minted this filing. **Pass ceiling: `Pass 305.0` → `Pass 306.0`**, next free family `307`.

### 2026-09-15 (557th filing, `Pass 307.0`, `729cf6db`) — decision 158: A TAB-VISIT SEQUENCE'S MEMBERSHIP RULE FOLLOWS §12.5.3'S INTERACTION BAR (HIDDEN/NOVIEW EXCLUDED, TOGGLENOVIEW REVERSAL HONOURED), NOT THE REQUEST'S UNQUALIFIED "EVERY ANNOTATION, FILTERED BY THE CALLER"

**Status: DECIDED / RECORDED.** `Pass 307.0` shipped `EditSession::page_tab_sequence(page_index) -> TabSequence`, computing a VISIT order for every `/Tabs` state (`/A`, `/W`, `/R`, `/C`, `/S`, `Absent`, `Other`) that `array_order_governs` could previously only classify.

**What the request assumed, and where §12.5.1 actually stops.** `pdfcer-gui`'s request (`request_G019_no_derived_tab_order_for_Tabs_R_C_or_S.md`, O204) asked for every annotation on the page in the returned order, with the caller responsible for filtering by kind. It was right that §12.5.1 defines no geometry, tolerance, corner or tie-break for the derived order at all — settled this Pass by `pdfcer-spec-librarian`'s new `iso32000__ref__tab_order_derivation.md` (`TABD-*` namespace, disjoint from the crate's existing `TAB-*` citations), a graded negative result with three controls behind it. pdfcer's `/R`/`/C` grouping rule is therefore an invention, and `TabSequence::notes` says so on every call that used it, rather than presenting a guess as something read off the file.

**Where the request was half wrong.** §12.5.1 is silent on the visit ORDER, but §12.5.3 is not silent on MEMBERSHIP: an annotation with the `Hidden` or `NoView` flag set "shall not… allow it to interact with the user," and being tabbed to is interaction. `TabSequence::order` excludes both flags' bearers; the exclusion is carried in `excluded`, split into two tiers so a reader can tell sourced rule from house judgement — `Hidden`/`NoView` (sourced, §12.5.3) and `TrapNet`/`Popup` (pdfcer's own reading: neither is a tabbable UI element by construction, and nothing in §12.5.3 addresses either by name).

**The one carve-out, and why it survives the exclusion above it.** An annotation with `NoView` **and** `ToggleNoView` both set is NOT excluded. ISO 32000-2 defines `ToggleNoView` as inverting `NoView`'s effect specifically for *annotation selection*, and tabbing to a widget is what selects it for input — the same reading `R167`'s editor-chrome carve-out already applies to on-canvas selection, extended here to the keyboard route into the same state.

**Body-section effects.** No existing `ARCHITECTURE.md` body section names annotation tab-visit membership; this entry is the first record of the rule, so there is nothing to amend beyond this decision log entry itself.
