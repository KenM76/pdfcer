...
| `ocrcer-core` | MIT | **OCRcer OCR engine** (`ocrcer` feature, default ON; `ocrs` stays the default ENGINE): a second recogniser, selected with `pdfcer ocr --ocr-engine ocrcer`. Pure safe Rust, zero dependencies, no I/O, compiles for wasm32. **Vendored** at `vendor/ocrcer-core` from the newest LOCAL OCRcer commit by `tools/sync-ocrcer.py` (operator rule, decision 160: local runs ahead of GitHub); `tools/check-ocrcer-vendored.py` fails the sweep when the copy is stale. Its model file `ocrcer.ocrw` is not a Cargo dependency, is not shipped, and carries no third-party content in the core model (OCRcer `NOTICE`; its prototype feature vectors derive from rendered OFL-1.1, Apache-2.0 and MIT faces, each listed in the model's `meta` block). |
