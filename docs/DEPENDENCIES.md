...
| `ocrcer-core` | MIT | **OCRcer OCR engine** (`ocrcer` feature, **opt-in, default OFF**; `Pass 327.0`): a second recogniser, selected with `pdfcer ocr --ocr-engine ocrcer`. Pure safe Rust, zero dependencies, no I/O, compiles for wasm32. **Local path dependency** (`../OCRcer`) until OCRcer is published. Its model file `ocrcer.ocrw` is not a Cargo dependency, is not shipped, and carries no third-party content in the core model (OCRcer `NOTICE`; no font data ships in the file — its prototype feature vectors are derived from rendered OFL-1.1, Apache-2.0 and MIT faces, each face and its licence listed in the model's `meta` block). |
