...
- **10 tests silently SKIP and report as passed** — ~~26~~ → ~~16~~ → **10**,
  ...
  **stayed green**. ★★ **The only fix is a synthetic fixture, and that is a
  ...
  ★★★ **THE NEXT THREE ARE BLOCKED ON A FIXTURE NOBODY CAN GENERATE — analysis
  done 2026-09-12, do not repeat it.** `editable_roundtrip.rs` (2) and
  `structure_inspect.rs` (1) need `external/qpdf/.../big-ostream.pdf`, and all
  they actually require is *a document that USES object streams*
  (`layout(&doc).object_streams` non-empty). Three facts, measured:
  - **pdfcer's writer cannot produce one.** It only ever DEcompresses — every
    `/Type /ObjStm` is promoted to file level on save (`writer/save.rs:1018`).
    So the fixture cannot be generated by round-tripping through this crate,
    which is the obvious first idea.
  - **No synthetic fixture in the repo contains one.** Checked every
    `fixtures/synthetic/**/*.pdf` for `/Type /ObjStm`: zero.
    `xref-recover/xref-stream-corrupt.pdf` mentions it and is deliberately
    damaged, so it is not a substitute.
  - ⇒ It must be **hand-authored**: an `/ObjStm` with `/N`, `/First` and its
    pair table, plus a cross-reference **stream** carrying type-2 entries.
    That is exacting enough that a subtly wrong fixture would make the tests
    pass for the wrong reason — the failure this whole exercise exists to
    prevent. Budget it properly rather than squeezing it in.
