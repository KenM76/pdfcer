...
| **Covers** | `EditSession` end to end: construction, the command/undo/redo model, **all 205 public methods**, the `EditError` taxonomy, the save path (incremental vs full rewrite), the guard/refusal model (encryption, certification, sidecar version, `/Size` suppression), object allocation and byte staging |
...
## 1. Verb index — all 205 public `EditSession` methods

**Count: 205.** Established by brace-matched extraction of the five
