...
| **Covers** | `EditSession` end to end: construction, the command/undo/redo model, **all 208 public methods**, the `EditError` taxonomy, the save path (incremental vs full rewrite), the guard/refusal model (encryption, certification, sidecar version, `/Size` suppression), object allocation and byte staging |
...
## 1. Verb index — all 208 public `EditSession` methods

**Count: 208.** Established by brace-matched extraction of the five
...
>
> The bytes returned are the signed file. The session still holds the staged
> placeholder (zeros in `/Contents`, sentinel `/ByteRange`), so **re-open the
> returned bytes** (`Document::from_bytes`) before any further edit — a later
> edit must append *after* this signature, and only a session on the signed
> file knows where that is. A CLI writes and exits; a GUI reloads. The staged
> `CommandKind::AddSignatureField` in the old session is harmless to undo.
>
> **Key sources.** `sign::Signer` is the one trait every digital-ID source
> reduces to (hash in, signature out, plus the certificate chain).
> `pdfcer-core` ships `sign::pkcs12::Pkcs12Signer` (a `.pfx`/`.p12`; both
> RFC 7292 encryption eras; the MAC is the password check). A Windows-store
> or PKCS#11 signer is a **shell-side** impl (`Pass 10.10`) — the key never
