...
| **Tesseract** 5.5.2 + Leptonica 1.87.0 | Apache-2.0 / BSD-2; the build's closure is all permissive (BSD, MIT, zlib, libpng, libtiff, IJG, 0BSD liblzma) | **ADOPTED as a subprocess, CLI only, 2026-09-25** (`--ocr-engine tesseract`) | Not a Cargo dependency and never linked: `pdfcer` spawns `models/tesseract/tesseract.exe` and core only parses its TSV (`ocr::tesseract_tsv`), so core stays process-free and wasm32-clean. The **default Windows builds are not shippable** (MinGW DLLs incl. LGPL libunistring/libiconv/libintl via curl; survey §4.3), so pdfcer builds its own: `tools/tesseract/build-tesseract.py`, a vcpkg overlay with curl, libarchive and ScrollView off, static MSVC triplet. Measured: one 5.37 MB exe importing **only KERNEL32.dll**; the script refuses any other import. Licence texts ship in `models/tesseract/LICENSES/`, attributed in `about.hbs`; not visible to `cargo-about`. Precedent: NAPS2 ships a static MSVC `tesseract.exe` the same way (with curl/libarchive still linked). |
...
- **2026-09-24 (`Pass 327.0`, 589th filing, decision 159)** — `ocrcer-core` (MIT, OCRcer
  project by the same operator) adopted as an **opt-in** second OCR engine,
  feature `ocrcer`, default OFF, by operator directive — an exception to the
  default-ON strippable-capability convention. Zero transitive dependencies;
