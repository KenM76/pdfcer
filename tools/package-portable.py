...
* **Every build gets its OWN dated folder**, never an overwrite. Two
  ...
  replaced**. Overwriting a folder whose `pdfcer.exe` is running fails
...
``D:\\builds\\pdfcer-<YYYYMMDD-HHMM>-<shorthash>[-dirty]\\``

    pdfcer.exe            the CLI (the GUI is the separate pdfcer-gui
...
#: The shipped binaries. `pdfcer` is a first-class feature, not a debug
...
def previous_build_commit(dest: Path) -> str | None:
    ...
    """
    builds = sorted(
        (p for p in dest.glob("pdfcer-*") if p.is_dir()),
...
def main() -> int:
    ...
    if not (REPO / "Cargo.toml").is_file():
        print(f"package-portable: {REPO} is not the pdfcer repo root.")
        return 1
    ...
    name = f"pdfcer-{stamp}-{short}" + ("-dirty" if dirty else "")
    out = args.dest / name
    ...
    #
    # ★ WITHOUT THIS BLOCK, `pdfcer ocr` AND THE GUI'S RECOGNISE BUTTON
    ...
    copied_assets = []
    assets_src = REPO / "crates" / "pdfcer-core" / "assets" / "models"
...
pdfcer.exe is a first-class scriptable CLI, not a debug tool — run it
with --help, or `pdfcer <subcommand> --help`. The desktop GUI is a
separate project (pdfcer-gui) and is not in this folder.

YOUR SETTINGS LIVE IN THIS FOLDER
---------------------------------
On first run pdfcer creates `userdata/` here and keeps your settings in
`userdata/settings.txt` (a plain, hand-editable `key = value` file).

That means: to update, replace the binaries but KEEP `userdata/`. If
pdfcer cannot write here — a read-only share, or Program Files without
