...
//!
//! ## Detection versus prevention — the distinction that changes pdfcer's behaviour
//!
...
//!
//! pdfcer is a consumer application. So:
//!
//! - **`/Perms → /DocMDP` present** ⇒ enforcement is a `shall`. pdfcer
...
//!   rather than performing it and warning. A warning would be pdfcer
//!   declining to do something the spec says it shall do.
//! - **DocMDP in `/Reference` but no `/Perms` entry** ⇒ detection only.
//!   pdfcer performs the edit and reports that it invalidates.
//!
...
//! page tree and is genuinely permitted at P≥2. pdfcer has no
//! template-instantiation feature, so no pdfcer operation can currently
//! land in that cell — but the rule is stated as *"none of pdfcer's
...
//!
//! pdfcer reports [`SignatureImpact::Invalidated`] for that case anyway,
...
//! under-reporting is pdfcer making a silent claim about the integrity of
...
//! signature does have a stage 2, just a field-scoped one). pdfcer records
...
//! not resolve it silently in code, so pdfcer uses neither.
...
///
/// pdfcer policy (`ARCHITECTURE.md` §10), not spec. The census walks the
...
/// Maximum depth of the AcroForm field tree walk (pdfcer policy).
///
...
pub enum SignatureImpact {
    ...
    /// pdfcer's conservative report for a plain approval signature, which
...
impl SignatureImpact {
    ...
    /// pdfcer's conservative-reporting policy.
    ///
    ...
    /// one is pdfcer declining to make a silent claim it cannot support.
    #[must_use]
...
pub enum ImpactBasis {
    ...
    SpecSourced,
    /// ISO 32000-1 is **silent**, and pdfcer reports the cautious answer
...
pub struct SignatureCensus {
    ...
    /// changes what pdfcer *does* rather than what it *says*: with it
...
impl SignatureCensus {
    ...
    /// make pdfcer warn about destroying something that does not exist.
    #[must_use]
...
mod tests {
    ...
    fn sig_flags_alone_is_a_declaration_and_not_a_signature() {
        ...
        // pdfcer warn about destroying a signature that does not exist.
    ...
    fn an_approval_signature_invalidation_is_a_conservative_report_not_a_citation() {
        ...
        // pdfcer's rule-4 policy and must be labelled as such.
        let doc = build(
    ...
    fn a_missing_transform_permission_defaults_to_two_not_one() {
        ...
        // Reading it as "maximally locked" would make pdfcer refuse edits
        // the signer allowed.
        let doc = build(
            "/AcroForm << /Fields [3 0 R] >> ",
            &[
...
/// what it would be valid *over*. Those are different claims and pdfcer
