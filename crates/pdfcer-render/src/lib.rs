...
// streams and images. Tests opt out per module. A per-pixel loop whose bound
// holds by construction may allow `indexing_slicing` on its smallest item,
// with the bound stated beside it.
#![deny(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
